...
@section('content')
                    ...
                    <div class="border-t border-gray-100 pt-4">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <input type="password" name="current_password" id="inputCurrentPassword" placeholder="Masukkan kata sandi lama Anda"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-all" />
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h4 class="text-sm font-bold text-gray-900 mb-1">Hapus Akun Toko</h4>
                        <p class="text-xs text-gray-500 max-w-md">Setelah Anda menghapus akun, semua data produk, riwayat transaksi, dan informasi toko akan dihapus secara permanen dari server kami.</p>
                    </div>
                    <form id="deleteAccountForm" action="{{ route('profile.delete') }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDeleteAccount()"
                            class="w-full md:w-auto px-6 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-red-100 active:scale-95">
                            Hapus Akun Permanen
                        </button>
                    </form>
                </div>
    ...
    <script>
        ...
        function checkFormChanged() {
            ...
            const currentPassChanged = document.getElementById('inputCurrentPassword') && document.getElementById('inputCurrentPassword').value.length > 0;
        ...
        // ===== SweetAlert2 Konfirmasi Hapus Akun =====
        function confirmDeleteAccount() {
            Swal.fire({
                title: 'Hapus Akun Permanen?',
                text: "Semua data produk dan toko Anda akan dihapus selamanya. Pastikan tidak ada pesanan yang sedang diproses.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
